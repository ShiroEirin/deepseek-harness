...
# PATH that calls through `current\apps\cli\src\bin.ts` (source launch via tsx).
# Windows has no symlinks for
...
# --- in-repo adoption --------------------------------------------------------
# Running from inside a checkout (this file's parent is a scripts/ dir with an
# apps/cli entry at the repo root): never clone, adopt the checkout as master.
$repoRoot = Split-Path -Parent $PSScriptRoot
$inRepo = (Test-Path (Join-Path $repoRoot 'apps\cli\src\bin.ts')) -and (Test-Path (Join-Path $repoRoot '.git'))
...
if (-not (Test-Path (Join-Path $DSH_STAGING 'apps\cli\src\bin.ts'))) {
  Fail "launcher $DSH_STAGING\apps\cli\src\bin.ts missing after install - is DSH_REF a branch that ships apps/cli?"
}
...
$shim = "@echo off`r`n" +
  ...
  "node --import tsx/esm `"$DSH_CURRENT\apps\cli\src\bin.ts`" %*`r`n" +
  "exit /b %ERRORLEVEL%`r`n"
Set-Content -Path (Join-Path $DSH_BIN_DIR 'dsh.cmd') -Value $shim -Encoding Ascii
Write-Info "wrote $DSH_BIN_DIR\dsh.cmd -> $DSH_CURRENT\apps\cli\src\bin.ts (source launch)"
