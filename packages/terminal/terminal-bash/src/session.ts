...
export class LocalPtySession implements TerminalBackendSession {
  ...
  constructor(
    ...
    this.scrollback = new BoundedTextBuffer(
      config.scrollbackMaxBytes,
  ...
  private async pollReadiness(operation: LocalSendOperation): Promise<void> {
    ...
    try {
      ...
      const startupHasOutput =
        !this.initializing || this.scrollback.snapshot().text.length > 0;
      const acceptsStdinWait =
        startupHasOutput &&
        foreground !== undefined &&
        operation.acceptsStdinWait(
          foreground.processGroupId,
          foreground.inputWaiting
        );
  ...
  private async drainTerminalProtocol(): Promise<void> {
    for (;;) {
      ...
      if (
        emulatorWrites === this.emulatorWrites &&
        responseWrites === this.responseWrites &&
        !this.protocolWorkPending()
      )
        return;
    }
  ...
  private async inspectForegroundAfterProtocol(): Promise<
    SubprocessTerminalForeground | undefined
  > {
    for (;;) {
      if (this.protocolWorkPending()) await this.drainTerminalProtocol();
      const emulatorWrites = this.emulatorWrites;
  ...
  private protocolWorkPending(): boolean {
    return (
      this.emulatorWriteDone !== undefined || this.pendingResponseWrites > 0
    );
  }
