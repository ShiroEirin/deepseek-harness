...
describe('real Loader composition', () => {
  ...
  it(
    ...
    async () => {
      ...
      expect(await request(port, '/probe')).toMatchObject({
        status: 200,
        body: 'EXACT',
      })
      expect(await request(port, '/api/anything')).toMatchObject({
        status: 200,
        body: 'API',
      })
      expect(await request(port, '/api/deep/leaf')).toMatchObject({
        status: 200,
        body: 'DEEP',
      })
      expect(await request(port, '/api')).toMatchObject({
        status: 200,
        body: 'API',
      })
      expect(
        await request(port, '/api/anything', { method: 'POST' }),
      ...
      expect(() => server.registerFallback(() => {})).toThrow(
        /fallback already registered/,
      )
      expect((await request(port, '/no/such/route')).body).toContain('__T__')
      untap()
      expect((await request(port, '/no/such/route')).body).not.toContain(
        '__T__',
      )
      ...
      expect(() =>
        server.register({ kind: 'exact', path: '/probe', handler: () => {} }),
      ).toThrow(/duplicate exact route/)
      const disposeOnce = server.register({
        kind: 'exact',
        path: '/once',
        handler: (_req, res) => {
          res.writeHead(200)
      ...
      expect(await request(port, '/once')).toMatchObject({
        status: 200,
        body: 'ONCE',
      })
      disposeOnce()
      ...
      expect(() =>
        server.register({ kind: 'exact', path: '/once', handler: () => {} }),
      ).not.toThrow()
      ...
      expect(() =>
        server.registerUpgrade({ path: '/events', handler: () => {} }),
      ...
      failedUpgrade.on('error', () => {
        /* The server-side reset is the fixture outcome. */
      })
  ...
  it(
    ...
    async () => {
      ...
      try {
        ...
        second = context
        expect(String(failure)).toMatch(/webserver: port .* is already in use/)
        expect(String(failure)).toContain(String(takenPort))
      } finally {
