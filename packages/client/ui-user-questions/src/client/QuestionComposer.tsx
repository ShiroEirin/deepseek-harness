...
export function QuestionComposer(props: QuestionComposerProps) {
  ...
  const question = useMemo(
    () => new PendingQuestion(props.matched),
    [props.matched]
  );
...
function QuestionFlow({
  pending,
  t,
}: { pending: PendingQuestion } & Pick<QuestionComposerProps, "t">) {
  const questions = pending.questions;
  const [index, setIndex] = useState(0);
  const [drafts, setDrafts] = useState<DraftAnswer[]>(() => {
    const cached = draftCache.get(pending.key);
    if (cached !== undefined && cached.length === questions.length)
      return cached;
  ...
  const completed = (item: DraftAnswer): boolean =>
