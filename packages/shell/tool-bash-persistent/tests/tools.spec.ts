...
class StubPtySession implements TerminalBackendSession {
  ...
  startSend(request: TerminalSendRequest): TerminalSendOperation {
    ...
    if (request.text.startsWith("stty -echo")) {
      ...
      return this.operation(
        Promise.resolve(this.result(this.motd, "stdin_read"))
      );
    }
    if (this.mode === "send-error") throw new Error("stub send failed");
    if (this.mode === "wait-for-abort" || this.mode === "end-on-abort") {
      const done = new Promise<ReturnType<StubPtySession["result"]>>(
        (resolve) => {
          request.signal?.addEventListener(
            "abort",
            () => {
              const start =
                /__DSH_PERSISTENT_BASH_START_[^_]+(?:-[^_]+)*__/.exec(
                  request.text
                )?.[0];
              const end = /__DSH_PERSISTENT_BASH_END_[^:]+:/.exec(
                request.text
              )?.[0];
              const output =
    ...
    if (this.mode === "prompt-after-idle") {
      if (request.text.length > 0) {
        const start = /__DSH_PERSISTENT_BASH_START_[^_]+(?:-[^_]+)*__/.exec(
          request.text
        )?.[0];
        const output = `${start ?? ""}\npartial syntax output\n`;
        this.scrollback += output;
        return this.operation(
          Promise.resolve(this.result(output, "inferred_idle"))
        );
      }
    ...
    if (
      this.mode === "exit" ||
      this.mode === "signal-exit" ||
      this.mode === "unknown-exit"
    ) {
      ...
      return this.operation(
        Promise.resolve(this.result(exitedOutput, "session_exit"))
      );
    }
    return this.operation(Promise.resolve(this.result(output, "stdin_read")));
  }
  ...
  read(request: TerminalReadRequest) {
    ...
    return {
      ...
      totalLines:
        this.mode === "empty-page-after-latest"
          ? lines.length + 1
          : lines.length,
      lineBegin: 0,
  ...
  private operation(
    done: Promise<ReturnType<StubPtySession["result"]>>,
    delta = ""
  ): TerminalSendOperation {
    return {
      done,
...
describe("tool-bash-persistent", () => {
  it("registers a configurable schema and reuses one owner shell", async () => {
    ...
    expect(ctx.tools.get("bash")?.presentCall?.({ command: "pwd" })).toEqual({
      card: "terminal",
      title: "pwd",
    });
    ...
    expect(text(await call(ctx, ownerWithoutCwd, "pwd"))).toBe(
      "hello from stub"
    );
  ...
  it("handles inferred idle, stdin_read fallback, shell exit, clipping, and cleanup", async () => {
    ...
    expect(text(await call(ctx, owner, "silent then complete"))).toContain(
      "hello from"
    );

    session.mode = "incremental-fallback";
    session.scrollback = "";
    expect(text(await call(ctx, owner, "incremental fallback"))).toContain(
      "increment"
    );
    ...
    expect(missingStart).toContain(
      "beginning of this command output was dropped"
    );
    ...
    expect(text(await call(ctx, owner, "kill shell"))).toContain(
  ...
  it("waits for status digits after a torn completion marker", async () => {
    ...
    expect(text(await call(ctx, owner, "torn status"))).toBe(
      "hello from stub\n[exit code: 7]"
    );
  });
  ...
  it("reports a shell exit when the backend has no code or signal", async () => {
    ...
    expect(text(await call(ctx, owner, "exit without status"))).toContain(
      "[shell exited]"
    );
  });
  ...
  it("marks a short missing-prefix result and tolerates exhausted scrollback pages", async () => {
    ...
    session.mode = "end-only";
    session.scrollback = "";
    expect(text(await call(ctx, owner, "missing start"))).toContain(
      "beginning of this command output was dropped"
    );

    session.mode = "empty-read";
    expect(text(await call(ctx, owner, "empty page"))).toContain(
      "hello from stub"
    );

    session.mode = "stalled-read";
    expect(text(await call(ctx, owner, "stalled page"))).toContain(
      "hello from stub"
    );

    session.mode = "empty-page-after-latest";
    expect(text(await call(ctx, owner, "empty continuation page"))).toContain(
      "hello from stub"
    );
  });
  ...
  it("assembles retained output across backward scrollback pages", async () => {
    ...
    expect(text(await call(ctx, owner, "paged output"))).toBe(
      "hello from stub"
    );
  });
  ...
  it("closes a timed-out shell and reports bounded partial output", async () => {
    ...
    expect(text(result)).toContain(
      "timed out after 0 seconds or experienced an OOM error"
    );
    expect(text(result)).toContain("partial output");
    expect(text(result)).toContain("next bash call starts from the workspace");
    expect(stub.sessions[0]?.closed).toContain(
      "persistent bash command timed out"
    );
  });
  ...
  it.each(["init-exit", "init-timeout"] as const)(
    ...
    async (mode) => {
      ...
      expect(stub.sessions[0]?.closed).toContain(
        "persistent bash initialization failed"
      );
    }
  );
  ...
  it("rejects invalid config and invalid calls", async () => {
    ...
    expect(text(await call(ctx, owner, " "))).toContain(
      "command must be a non-empty string"
    );

    const controller = new AbortController();
    controller.abort(new Error("caller stopped"));
    expect((await call(ctx, owner, "pwd", controller.signal)).isError).toBe(
      true
    );
    expect(stub.sessions).toHaveLength(0);
