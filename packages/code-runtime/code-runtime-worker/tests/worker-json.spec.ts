...
describe('snapshotCodeJsonValue', () => {
  ...
  it('detaches dense arrays and plain or null-prototype records', () => {
    ...
    const snapshot: Record<string, unknown> = snapshotCodeJsonValue(source) as Record<string, unknown>
    shared.value = 2
  ...
  it('reads each accepted slot once and preserves a literal __proto__ key', () => {
    ...
    const snapshot: Record<string, unknown>[] = snapshotCodeJsonValue(array) as Record<string, unknown>[]
